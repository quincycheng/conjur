...
def gce_identity_access_token(token_type)
  token_file_name = 'gce_token_' << token_type.to_s

  case token_type
  when :valid
    @gce_token_valid ||= read_token_file(token_file_name)
    @gce_identity_token = @gce_token_valid
  when :standard_format
    @gce_token_standard_format ||= read_token_file(token_file_name)
    @gce_identity_token = @gce_token_standard_format
  when :invalid_audience
    @gce_token_invalid_audience ||= read_token_file(token_file_name)
    @gce_identity_token = @gce_token_invalid_audience
  when :non_existing_host
    @gce_token_non_existing_host ||= read_token_file(token_file_name)
    @gce_identity_token = @gce_token_non_existing_host
  when :non_existing_account
    @gce_token_non_existing_account ||= read_token_file(token_file_name)
